...
For Utility Network projects, the toolkit supports a safer and more repeatable way to migrate newly created or recently edited records from a legacy source system into a staging or quality-control environment before reconciliation into production.
...
- Applying edit-date filters to isolate recently changed records.
- Loading records into an intermediate target workspace.
- Appending results into a versioned Utility Network environment.
...
- Date-filtered data extraction.
- Intermediate asset package loading.
- Versioned Utility Network append workflows.
...
3. Apply an edit-date or modified-date filter to identify incremental changes.
...
6. Append staged records to a versioned Utility Network target.
7. Generate logs and structured run summaries.
8. Validate loaded records before production reconciliation.
9. Produce QA/QC reports for analysts and project leads.
10. Optionally notify designated reviewers when errors occur.

The toolkit is intended to support a controlled migration pattern: load into a staging or named version first, review the result, then reconcile into production only after QA/QC is complete.
...
| Edit-date window | Recommended | Start/end date or modified-date expression used to capture records created or updated during a specific period. |
| Target version or staging environment | Recommended | Named QA/QC version, branch version, or staging target where migrated records can be reviewed before production reconciliation. |
...
| Output | Description |
| --- | --- |
| Updated data reference table | A run-specific data loading table with refreshed paths, target references, and edit-date filters. |
| Staging workspace | Intermediate geodatabase or asset package containing transformed records before append. |
| Versioned append result | Records loaded into a QA/QC version or staging Utility Network target. |
...
| Reconciliation handoff package | Summary information that helps reviewers decide whether the version is ready for reconcile/post. |
...
| --- | --- | --- |
| Source dataset inventory | Yes | List of source feature classes, tables, feature counts, edit-date filters, and object identifiers included in the run. |
...
| --- | --- |
| Completeness summary | Compares source records selected by the edit-date filter against records loaded into the staging or target environment. |
...
Data Reference Table + Migration Config
        |
        v
Edit-Date Filtered Extraction
        |
        v
Staging Workspace / Empty Target Schema
        |
        v
Versioned Utility Network Append
        |
        v
QA/QC Validation Report
        |
        v
Reviewer Decision: Fix, Re-run, or Reconcile
```
...
reference = prepare_reference_table(
    ...
    edit_date_field="last_edited_date",
...
- A refreshed data reference table for the selected time window.
- A staging workspace containing only the records selected for migration.
- A versioned Utility Network append result that can be reviewed before production reconciliation.
...
Planning and initial documentation stage. The first implementation milestone is a Python package skeleton with a data reference parser, edit-date filter generator, validator, and sample QA/QC report.
